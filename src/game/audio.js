...
class AudioEngine {
  ...
  init() {
    ...
    // gentle master compression so particle-storm moments don't clip
  ...
  startPad() {
    ...
    this.padBus.gain.linearRampToValueAtTime(0.5, t + 6);
  }
  ...
  tone({ type = 'sine', freq = 440, to = null, a = 0.005, d = 0.2, peak = 0.2, glideTime = 0.1, pan = 0 }) {
  ...
  noise({ dur = 0.3, peak = 0.2, freq = 1200, q = 1, type = 'bandpass', slideTo = null }) {
    if (!this.ctx || !this.enabled) return;
    const t = this.ctx.currentTime;
